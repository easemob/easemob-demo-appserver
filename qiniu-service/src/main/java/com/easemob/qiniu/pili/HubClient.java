...
public final class HubClient {
     ...
     * @throws PiliException
     */
    public Stream.LiveStatus liveStatus(String streamTitle) throws PiliException {
        String path = String.format("%s/streams/%s/live", baseUrl, UrlSafeBase64.encodeToString(streamTitle));
        try {
            String resp = cli.callWithGet(path);
            return gson.fromJson(resp, Stream.LiveStatus.class);
        } catch (PiliException e) {
            throw e;
        } catch (Exception e) {
            throw new PiliException(e);
